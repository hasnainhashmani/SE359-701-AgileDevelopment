...
public class RobotDummy {
	...
	private void gatherData(){
		...
		for(Point p:visible){
			...
			known.addTile(p, new Tile(toExplore.getFloorTypeAt(p), 1, toExplore.floorIsObstacle(p), p)); //set dirt to 1 unless we've explored it
			}
	...
	public List<Point> getPath(Point from, Point to){
		...
		Pathfinder pf = new Pathfinder(known);
		pf.findPath(from, to);
		
		List<Point> path = pf.RetracePath();	
		List<Point> temp = new ArrayList<Point>();
		
		System.out.println("Start from point: " + from);
